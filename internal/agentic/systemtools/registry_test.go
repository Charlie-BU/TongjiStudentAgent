...
func TestSystemToolsRequireToolAllowlist(t *testing.T) {
	Convey("静态系统工具注册", t, func() {
		Convey("未加白工具不应注册", func() {
			So(buildTools(func(string) bool { return false }, nil, nil, &tavily.Client{}, &webfetch.Client{}), ShouldBeEmpty)
		})
		...
		Convey("网页能力需要客户端且逐项加白", func() {
			...
			tools := buildTools(func(name string) bool { return name == toolallowlist.URLFetchTool }, nil, nil, nil, &webfetch.Client{})
			So(tools, ShouldHaveLength, 1)
			info, err := tools[0].Info(context.Background())
			So(err, ShouldBeNil)
			So(info.Name, ShouldEqual, toolallowlist.URLFetchTool)
		})
		Convey("Tavily 与自适应抓取客户端独立注册", func() {
			searchOnly := Tools(WithTavilyClient(&tavily.Client{}))
			So(searchOnly, ShouldHaveLength, 2)
			fetchOnly := Tools(WithWebFetchClient(&webfetch.Client{}))
			So(fetchOnly, ShouldHaveLength, 2)
		})
