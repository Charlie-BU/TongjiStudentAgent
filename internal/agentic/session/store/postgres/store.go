...
func (s *PostgresStore) ListMessages(ctx context.Context, sessionID, ownerUserID string, limit int) ([]Message, error) {
	...
	rows, err := s.pool.Query(ctx, `SELECT id, session_id, run_id, sequence, role, content, tool_calls, tool_call_id, tool_name, reasoning_content, response_id, response_cache_expires_at, model_tier, model_id, protocol_data, created_at FROM agent_session_messages WHERE session_id = $1 ORDER BY sequence DESC LIMIT $2`, strings.TrimSpace(sessionID), limit)
	if err != nil {
	...
	for rows.Next() {
		var message Message
		var toolCalls []byte
		if err := rows.Scan(&message.ID, &message.SessionID, &message.RunID, &message.Sequence, &message.Role, &message.Content, &toolCalls, &message.ToolCallID, &message.ToolName, &message.ReasoningContent, &message.ResponseID, &message.ResponseCacheExpiresAt, &message.ModelTier, &message.ModelID, &message.ProtocolData, &message.CreatedAt); err != nil {
...
`,
	...
	`ALTER TABLE agent_session_messages ADD COLUMN IF NOT EXISTS protocol_data TEXT NOT NULL DEFAULT '';`,
	`
