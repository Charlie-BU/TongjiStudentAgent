...
- 2026-09-23 19:59（Asia/Shanghai）

### Base Commit

- `d47633e57141cf3dbee32a4abc11e8660bd163b7`（沿用历史记录格式，取 `HEAD~1`，仅作基线元数据）。

### Compare Scope

- `working_tree_only`：全部当前未提交改动，相对 `HEAD`（`dc2e01e0b14a577c40068a85b9121f9f077d3964`）比较。MCP Server 的接口实现、工具人群元数据及此前已提交的服务凭据分离不作为本仓新增实现。

### 背景与改动目标

配合同济 MCP 工具扩展及命名调整，同步 Agent 的远程工具 allowlist，避免启动发现名单不完整或调用旧名称。同时更新课程与教师指南，明确全部学期与当前学期的查询入口。

### 改动概览

- 远程 MCP allowlist 从 36 个工具调整为 59 个：新增 25 个校园工具，移除 `tongji.user.basic_info` 和 `tongji.course.calendar_list`，并同步既有工具的新名称。
- 新增批准能力涵盖研究生成绩、学分与培养计划，科研项目/著作/专利，联系方式读取与修改，资助记录，教师课表及职称，一卡通余额，考试安排，本科绩点汇总，邮箱及辅导员。
- 将既有年度账单、校园卡流水、学期日历、借阅、统计与门禁工具改为 `tongji.user.*`；本科成绩和竞赛奖励改为 `tongji.bachelor.*`。课程评价与瑞幸工具继续保留。
- 课程与教师 Skill 删除旧 `tongji.course.calendar_list` 契约及调用指引，明确全部学期 ID 使用 `tongji.user.term-calendar({})`，当前学期使用 `tongji.user.current-term-calendar({})`。
- Skill 明确学期查询需要登录、ID 以实际返回为准，不能凭日期推测，也不能把课程搜索的 `term` 编码直接当作 `calendarId`。
- 更新聊天服务 allowlist 预期、MCP 调用测试、会话上下文及 PostgreSQL 测试中的工具名称，并移除 Skill 测试对已删除工具的引用。
- README 说明当前批准 59 个远程工具，以及 MCP 缺少名单内工具会导致启动完整性检查失败。

### 关键链路解析（含上下游）

- 工具装配：Agent 启动发现远程 MCP 工具，按更新后的 allowlist 校验并暴露完整的 59 个工具；工具本身的接口、输入输出与人群描述由配套 MCP 提供。
- 学期查询：加载课程与教师指南后，根据用户意图选择全部学期或当前学期工具；需要学期 ID 时使用返回值，课程搜索仍遵守其独立的学期编码契约。匿名或工具不可用时说明信息缺口。
- 测试配套：名单测试列入新工具，已有消息历史装配、存储与 MCP 调用测试使用新的本科成绩工具名。本次不实现已持久化历史消息中旧工具名的迁移。

### 改动结果与业务影响

- Agent 可发现配套 MCP 新增的校园能力，当前名单包含 42 个校园工具、6 个课程/历史评价工具及 11 个瑞幸工具。
- 学期查询指引区分“所有学期 ID”与“当前学期”，不再引导模型调用已移除的课程学期列表工具。
- Agent 与 MCP 需配套部署；旧版本 MCP 缺少新增或重命名工具时，Agent 启动完整性检查会失败。联系方式修改属于写操作，调用仍须遵守工具提供的明确意图及失败不自动重试要求。

### 风险与待办

- 此前本轮 MCP 审阅已将实际 tools/list 与同工作区 Agent allowlist 逐项比对：两者均为 59 个，无缺项或多余项。
- 本次仅生成 changelog，未运行 Agent Go 测试或真实模型/校园接口联调；不能将 MCP 仓测试结果视为 Agent 回归测试通过。文档差异检查通过。
- Skill 提供模型调用指引，不代表已验证真实模型必然正确选择学期工具；新增校园能力及写操作仍需受控端到端验证。

### 建议 Commit Message（git-cz）

- `feat(agent): sync campus tool allowlist and clarify term lookup guidance`
...
- `c0c37e26432340a5419d17897243705c2265c09e`（按规范取 `HEAD~1`，仅作基线元数据）。

### Compare Scope

- `working_tree_only`：当前未提交改动，相对 `HEAD`（`ac5d35668964ddaac3e632d6de5d19440f107327`）比较。此前已提交的模型环境变量更名不纳入本记录。

### 背景与改动目标

此前聊天统一使用 `LITE_MODEL`，已有的 `PRO_MODEL`、`MAX_MODEL` 配置尚未参与请求路由。这次让调用方在每轮消息中选择模型档位，同时保留默认 lite 的调用方式。模型切换还需要处理历史响应缓存：旧模型的响应引用不能直接交给新模型，因此路由、消息元数据和上下文缓存处理一起调整。

### 改动概览

- `biz/handler/agent.go` 为消息请求增加 `model_tier`。省略时使用 `lite`；显式值只接受小写字符串 `lite`、`pro`、`max`。空字符串、null、非字符串及其他值返回 HTTP 400；合法但未配置的档位返回 HTTP 503，不自动降级。校验在开启 SSE 和写入用户消息前完成。
...
- 上游依赖：服务从 `LITE_MODEL`、`PRO_MODEL`、`MAX_MODEL` 读取模型配置，并使用同一组 `ARK_API_KEY` 和 Ark 地址。lite 仍为必需配置，pro、max 留空时跳过创建；已配置档位的模型或 Runtime 创建失败会使初始化失败，并沿用已有资源清理路径。
- 当前改动：消息入口先校验档位，Chat 再选择本轮 Runtime，沿用会话锁与归属校验读取历史；过滤不兼容缓存后执行 Agent，并将本轮模型信息随消息写入对应存储。
- 下游影响：Runtime 和上下文组装器接收过滤后的历史，工具沿用现有装配方式。旧客户端无需新增参数即可继续使用 lite；需要选择 pro、max 的客户端须发送对应档位，并处理进入 SSE 前的 HTTP 400/503。历史消息读取会返回新写入消息的模型字段。

### 改动结果与业务影响

- 同一会话可逐轮选择不同模型，选择仅作用于本轮执行，不保存为下一轮的默认档位。
- 缓存隔离以模型 ID 为依据：不同档位指向同一模型时可继续复用兼容缓存；模型改变时使用现有历史窗口重建上下文，不删除持久化消息。
- 模型配置在启动时装配，修改后需要重启。增加已配置档位会增加常驻模型客户端和 Runtime；切换模型时失去原缓存复用，可能增加输入处理开销。

### 风险与待办

- 已验证：上一轮对同一代码改动执行的 `go test ./...`、`go vet ./...`、`go test -race ./biz/handler ./internal/application/chat ./internal/agentic/session/...` 及 `git diff --check HEAD` 均通过。本次仅生成文档，未重复执行 Go 测试。
- 未验证：真实 Ark 服务上的跨模型连续对话、工具调用和响应缓存行为，以及生产 PostgreSQL 的补列执行。部署前需确认所配置模型支持当前 Responses API、推理参数和工具调用配置。
- 建议补充带持久化历史的多轮切换联调，覆盖 lite → pro → lite、旧消息和工具结果；当前并发路由测试使用替身，缓存隔离测试单独验证过滤及上下文组装。
- 新增模型路由等测试尚未统一采用仓库要求的 GoConvey 场景结构，后续应按测试规约整理。

### 建议 Commit Message（git-cz）

- `feat(chat): route model tiers and isolate session response caches`
